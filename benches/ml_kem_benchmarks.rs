...
fn benchmark_keygen(c: &mut Criterion) {
    ...
    c.bench_function("ml_kem_768_keygen", |b| {
        b.iter(|| {
            let _keypair = ml_kem.generate_keypair()
...
fn benchmark_encapsulation(c: &mut Criterion) {
    ...
    c.bench_function("ml_kem_768_encapsulation", |b| {
        b.iter(|| {
            let (ciphertext, shared_secret) = ml_kem.encapsulate(keypair.public_key())
...
fn benchmark_decapsulation(c: &mut Criterion) {
    ...
    c.bench_function("ml_kem_768_decapsulation", |b| {
        b.iter(|| {
            let shared_secret = ml_kem.decapsulate(keypair.secret_key(), &ciphertext)
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    ...
                    for keypair in &keypairs {
                        let result = ml_kem.encapsulate(keypair.public_key())
                            .expect("Encapsulation failed");
                        results.push(result);
                    }
                    black_box(results);
    ...
    c.bench_function("ml_kem_768_memory_stress", |b| {
        b.iter(|| {
            ...
            for keypair in &keypairs {
                let (ciphertext, shared_secret) = ml_kem.encapsulate(keypair.public_key())
                    .expect("Encapsulation failed");
                let recovered_secret = ml_kem.decapsulate(keypair.secret_key(), &ciphertext)
                    .expect("Decapsulation failed");
