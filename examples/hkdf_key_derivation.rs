...
fn basic_hkdf_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_via_enum = KdfAlgorithm::HkdfSha3_256
        .derive(initial_key_material, Some(salt), info, 32)?;
...
fn key_stretching_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Different labels give different keys: {}\n", stretched != stretched2);
...
fn ml_kem_with_hkdf_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("ML-KEM-768 Shared Secret: {} bytes", shared_secret.as_bytes().len());
...
fn password_derivation_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Different salt gives different key: {}", derived_key != derived_key2);
...
fn extract_expand_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n✓ Extract+Expand equals Derive: {}", okm_direct == okm_two_phase);
