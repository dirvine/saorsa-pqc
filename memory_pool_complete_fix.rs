...
// Implement AsRef and AsMut traits properly instead of custom methods
impl<T: BufferCleanup> AsRef<T> for PoolGuard<T> {
    fn as_ref(&self) -> &T {
        self.get()
    }
}

impl<T: BufferCleanup> AsMut<T> for PoolGuard<T> {
...
impl PqcMemoryPool {
    ...
    /// # Errors
    ///
    /// Returns an error if the pool cannot allocate a buffer due to capacity limits or locking issues
...
pub fn initialize_global_pool() -> Result<(), Box<dyn std::error::Error>> {
    ...
    GLOBAL_POOL
        .set(pool)
        .map_err(|_| -> Box<dyn std::error::Error> { "Global pool already initialized".into() })?;
...
mod tests {
    ...
    fn test_all_buffer_types() {
        ...
        assert_eq!(ml_kem_pk.get().0.len(), ML_KEM_768_PUBLIC_KEY_SIZE);

        let ml_kem_sk = pool.acquire_ml_kem_secret_key().unwrap();
        assert_eq!(ml_kem_sk.get().0.len(), ML_KEM_768_SECRET_KEY_SIZE);

        let ml_kem_ct = pool.acquire_ml_kem_ciphertext().unwrap();
        assert_eq!(ml_kem_ct.get().0.len(), ML_KEM_768_CIPHERTEXT_SIZE);

        let ml_dsa_pk = pool.acquire_ml_dsa_public_key().unwrap();
        assert_eq!(ml_dsa_pk.get().0.len(), ML_DSA_65_PUBLIC_KEY_SIZE);

        let ml_dsa_sk = pool.acquire_ml_dsa_secret_key().unwrap();
        assert_eq!(ml_dsa_sk.get().0.len(), ML_DSA_65_SECRET_KEY_SIZE);

        let ml_dsa_sig = pool.acquire_ml_dsa_signature().unwrap();
        assert_eq!(ml_dsa_sig.get().0.len(), ML_DSA_65_SIGNATURE_SIZE);
    }
