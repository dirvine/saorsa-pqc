...
mod ml_dsa_validation {
    ...
    fn test_key_generation_compatibility() {
        ...
        assert_eq!(our_pk.as_bytes().len(), 1952, 
                   "Our public key size is incorrect");
        assert_eq!(our_sk.as_bytes().len(), 4032,
    ...
    fn test_signature_cross_validation() {
        ...
        assert_eq!(our_sig.as_bytes().len(), 3309,
    ...
    fn test_message_sizes() {
        ...
        for message in test_messages {
            ...
            assert!(ref_sig.is_ok(), "Reference signing failed for message size {}", message.len());
            
            let ref_sig = ref_sig.unwrap();
            let ref_verify = ref_pk.verify(&message, &ref_sig, &[]);
            assert!(ref_verify, "Reference verification failed for message size {}", message.len());
        }
    }
}
...
mod ml_kem_validation {
    ...
    fn test_kem_key_generation() {
        ...
        #[cfg(feature = "aws-lc-rs")]
        {
            ...
            assert_eq!(our_pk.as_bytes().len(), 1184,
                       "Our KEM public key size is incorrect");
            assert_eq!(our_sk.as_bytes().len(), 2400,
    ...
    fn test_kem_encap_decap() {
        ...
        assert_eq!(ref_ss.into_bytes(), ref_ss_dec.into_bytes(),
        ...
        #[cfg(feature = "aws-lc-rs")]
        {
            ...
            assert_eq!(our_ss.as_bytes(), our_ss_dec.as_bytes(),
                       "Our encap/decap mismatch");
            
            // Check sizes are correct (ML-KEM-768 sizes from FIPS 203)
            assert_eq!(our_ct.as_bytes().len(), 1088,
                       "Our ciphertext size is incorrect");
            assert_eq!(our_ss.as_bytes().len(), 32,
    ...
    fn test_multiple_encapsulations() {
        ...
        assert_ne!(ct1_bytes, ct2_bytes,
                   "Encapsulations should be randomized");
