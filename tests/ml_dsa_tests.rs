...
use common::{load_test_vectors, hex_to_bytes, TestVectorError};
use saorsa_pqc::pqc::ml_dsa::{MlDsa65, MlDsaKeyPair, MlDsaPublicKey, MlDsaSecretKey, MlDsaSignature};
...
fn test_ml_dsa_keygen_nist_vectors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(vectors.test_groups.len(), expected.test_groups.len(),
               "Mismatch between prompt and expected test groups");
    
    for (test_group, expected_group) in vectors.test_groups.iter()
        .zip(expected.test_groups.iter())
    {
        ...
        assert_eq!(test_group.tests.len(), expected_group.tests.len(),
                   "Mismatch between prompt and expected test cases");
        
        for (test, expected_test) in test_group.tests.iter()
            .zip(expected_group.tests.iter())
        {
            println!("Testing ML-DSA-65 KeyGen test case {}", test.tc_id);
            
            // Extract deterministic seed
            let seed = test.seed.as_ref()
                .ok_or("Missing seed in test case")?;
            ...
            let expected_pk = expected_test.pk.as_ref()
                .ok_or("Missing expected public key")?;
            let expected_sk = expected_test.sk.as_ref()
            ...
            assert_eq!(expected_pk_bytes.len(), 1952, 
                      "ML-DSA-65 public key must be 1952 bytes");
            assert_eq!(expected_sk_bytes.len(), 4032, 
            ...
            .zip(expected_group.tests.iter())
        {
            ...
            if let (Some(sk_hex), Some(message_hex)) = (&test.sk, &test.message) {
                ...
                if let Some(signature_hex) = &expected_test.signature {
                    ...
                    assert!(expected_signature.len() <= 3309,
                           "ML-DSA-65 signature too large: {} bytes", expected_signature.len());
        ...
        .zip(expected.test_groups.iter())
    {
            ...
            .zip(expected_group.tests.iter())
        {
                ...
                (&test.pk, &test.message, &test.signature) 
            {
                ...
                assert!(signature_bytes.len() <= 3309,
                       "Signature too large: {} bytes", signature_bytes.len());
            }
        }
    }
...
fn test_ml_dsa_sign_verify_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, message) in test_messages.iter().enumerate() {
        ...
        assert!(is_valid, "Signature verification should succeed for message {}", i);
        
        // Verify signature size bounds
        assert!(signature.as_bytes().len() <= 3309,
               "Signature too large for message {}: {} bytes", i, signature.as_bytes().len());
    }
...
fn test_ml_dsa_invalid_signature() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Sign the message
    let mut signature = ml_dsa.sign(keypair.secret_key(), message)
    ...
    assert!(!is_valid, "Signature verification should fail for corrupted signature");
    
    // Test 2: Verify original signature still works
    let is_valid = ml_dsa.verify(keypair.public_key(), message, &original_sig)
        .map_err(|e| format!("Verification failed: {:?}", e))?;
...
fn test_ml_dsa_wrong_message() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let signature = ml_dsa.sign(keypair.secret_key(), message1)
        .map_err(|e| format!("Signing failed: {:?}", e))?;
    
    // Try to verify with different message
    let message2 = b"Different message entirely";
    let is_valid = ml_dsa.verify(keypair.public_key(), message2, &signature)
        .map_err(|e| format!("Verification failed: {:?}", e))?;
    
    assert!(!is_valid, "Signature verification should fail for different message");
    
    // Verify original message still works
    let is_valid = ml_dsa.verify(keypair.public_key(), message1, &signature)
        .map_err(|e| format!("Verification failed: {:?}", e))?;
...
fn test_ml_dsa_key_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Generate keypair
    let original_keypair = ml_dsa.generate_keypair()
        .map_err(|e| format!("Key generation failed: {:?}", e))?;
    ...
    let signature = ml_dsa.sign(&restored_sk, message)
        .map_err(|e| format!("Signing with restored key failed: {:?}", e))?;
    
    let is_valid = ml_dsa.verify(&restored_pk, message, &signature)
...
fn test_ml_dsa_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(keygen_time.as_millis() < 200, 
           "Key generation too slow: {}ms", keygen_time.as_millis());
    assert!(sign_time.as_millis() < 100, 
           "Signing too slow: {}ms", sign_time.as_millis());
    assert!(verify_time.as_millis() < 50, 
           "Verification too slow: {}ms", verify_time.as_millis());
    ...
    for i in 0..4 {
        ...
        let handle = thread::spawn(move || -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            for j in 0..10 {
                ...
                let is_valid = ml_dsa_clone.verify(keypair.public_key(), message.as_bytes(), &signature)?;
                assert!(is_valid);
            }
