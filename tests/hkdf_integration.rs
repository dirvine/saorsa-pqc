...
use saorsa_pqc::{HkdfSha3_256, HkdfSha3_512, KdfAlgorithm, kdf_helpers};
...
fn test_hkdf_extract_and_expand() {
    ...
    HkdfSha3_256::expand(&prk, info1, &mut okm1)
        .expect("First expand should succeed");
    HkdfSha3_256::expand(&prk, info2, &mut okm2)
        .expect("Second expand should succeed");
    
    assert_ne!(okm1, okm2, "Different info should produce different outputs");
}
...
fn test_key_stretching() {
    ...
    let stretched1 = kdf_helpers::stretch_key(short_key, label1, 48)
        .expect("Key stretching should succeed");
    let stretched2 = kdf_helpers::stretch_key(short_key, label2, 48)
        .expect("Key stretching should succeed");
    
    assert_eq!(stretched1.len(), 48);
    assert_eq!(stretched2.len(), 48);
    assert_ne!(stretched1, stretched2, "Different labels should give different outputs");
}
...
fn test_password_key_derivation() {
    ...
    assert_ne!(&key1[..], &key3[..], "Different iterations should produce different keys");
}
...
fn test_hkdf_max_output_length() {
    ...
    assert!(result.is_err(), "Should fail when exceeding max output length");
}
