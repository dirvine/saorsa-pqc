...
use saorsa_pqc::pqc::ml_kem::{MlKem768, MlKemKeyPair, MlKemPublicKey, MlKemSecretKey, MlKemCiphertext, MlKemSharedSecret};
...
fn test_ml_kem_keygen_nist_vectors() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .zip(expected.test_groups.iter()) 
    {
            ...
            .zip(expected_group.tests.iter())
        {
            ...
            let d = test.d.as_ref()
                .ok_or("Missing d seed in test case")?;
            let z = test.z.as_ref()
                .ok_or("Missing z seed in test case")?;
            ...
            let expected_ek = expected_test.ek.as_ref()
                .ok_or("Missing expected encapsulation key")?;
            let expected_dk = expected_test.dk.as_ref()
            ...
            assert_eq!(expected_ek_bytes.len(), 1184, 
                      "ML-KEM-768 public key must be 1184 bytes");
            assert_eq!(expected_dk_bytes.len(), 2400, 
...
fn test_ml_kem_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Verify sizes
    assert_eq!(ciphertext.as_bytes().len(), 1088, "Ciphertext size incorrect");
    assert_eq!(shared_secret1.as_bytes().len(), 32, "Shared secret size incorrect");
...
fn test_ml_kem_invalid_ciphertext() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (mut ciphertext, _) = ml_kem.encapsulate(keypair.public_key())
        .map_err(|e| format!("Encapsulation failed: {:?}", e))?;
    ...
    assert_eq!(corrupted_secret.as_bytes().len(), 32, 
...
fn test_ml_kem_key_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Generate keypair
    let original_keypair = ml_kem.generate_keypair()
        .map_err(|e| format!("Key generation failed: {:?}", e))?;
    ...
    let (ciphertext, shared_secret1) = ml_kem.encapsulate(&restored_pk)
        .map_err(|e| format!("Encapsulation with restored key failed: {:?}", e))?;
    
    let shared_secret2 = ml_kem.decapsulate(&restored_sk, &ciphertext)
...
fn test_ml_kem_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(keygen_time.as_millis() < 100, 
           "Key generation too slow: {}ms", keygen_time.as_millis());
    assert!(encap_time.as_millis() < 50, 
           "Encapsulation too slow: {}ms", encap_time.as_millis());
    assert!(decap_time.as_millis() < 50, 
           "Decapsulation too slow: {}ms", decap_time.as_millis());
    ...
    for i in 0..4 {
        ...
        let handle = thread::spawn(move || -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            for _ in 0..10 {
                ...
                let (ciphertext, shared_secret1) = ml_kem_clone.encapsulate(keypair.public_key())?;
                let shared_secret2 = ml_kem_clone.decapsulate(keypair.secret_key(), &ciphertext)?;
                assert_eq!(shared_secret1.as_bytes(), shared_secret2.as_bytes());
            }
