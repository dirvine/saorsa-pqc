...
use saorsa_pqc::api::{
    MlKem, MlKemVariant, MlDsa, MlDsaVariant, SlhDsa, SlhDsaVariant,
    kem::ml_kem_768, dsa::ml_dsa_65, slh::slh_dsa_sha2_128s,
};
...
fn test_api_ml_kem_simple() {
    ...
    // Encapsulate
    let (shared_secret1, ciphertext) = kem.encapsulate(&public_key)
...
fn test_api_ml_dsa_simple() {
    ...
    let is_valid = dsa.verify(&public_key, wrong_message, &signature)
        .expect("Failed to verify signature");
...
fn test_api_ml_dsa_with_context() {
    ...
    assert!(dsa.verify_with_context(&pk, message, &signature, context).unwrap());
    
    // Verify with wrong context fails
    assert!(!dsa.verify_with_context(&pk, message, &signature, b"wrong-context").unwrap());
}
...
fn test_api_error_handling() {
    use saorsa_pqc::api::{MlKemPublicKey, MlDsaSecretKey, PqcError};
...
fn test_api_comprehensive_workflow() {
    ...
    assert!(dsa.verify(&dsa_pk, &ciphertext.to_bytes(), &signature).unwrap());
