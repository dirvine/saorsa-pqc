...
use fips203::{ml_kem_512, ml_kem_768, ml_kem_1024, traits::{Encaps, Decaps, KeyGen, SerDes as KemSerDes}};
...
fn test_ml_kem_768_basic() {
    ...
    let (ss1, ct) = ek.try_encaps()
        .expect("ML-KEM-768 encaps failed");
    
    let ss2 = dk.try_decaps(&ct)
...
fn test_ml_dsa_wrong_signature() {
    ...
    assert!(!pk.verify(msg2, &sig1, ctx),
...
fn test_cross_compatibility() {
    ...
    assert!(!pk2.verify(msg, &sig1, ctx),
