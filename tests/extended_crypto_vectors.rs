...
use saorsa_pqc::api::{
    ...
    aead::{Aes256GcmAead, AeadCipher, GcmNonce},
    hpke::HpkeConfig,
    traits::{Hash, Kdf, Mac, Aead},
    MlKemVariant,
};
use hex;
...
mod hmac_vectors {
    ...
    fn test_hmac_sha3_256_basic() {
        ...
        mac3.update(message);
        assert!(mac3.verify(tag.as_ref()).is_ok(), "HMAC verification should succeed");
    }
    ...
    fn test_hmac_sha3_256_rfc_test_case() {
        ...
        assert!(mac_wrong.verify(tag.as_ref()).is_err(), "Wrong key should fail verification");
    }
    ...
    fn test_hmac_truncated_verification() {
        ...
        let mut mac_verify = HmacSha3_256::new(key).unwrap();
        mac_verify.update(message);
        assert!(mac_verify.verify(truncated_tag).is_err(), "Truncated tag should fail full verification");
