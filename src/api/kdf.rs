...
impl Kdf for HkdfSha3_256 {
    ...
    fn expand(prk: &[u8], info: &[u8], okm: &mut [u8]) -> PqcResult<()> {
        let hkdf = HkdfImpl::<Sha3_256>::from_prk(prk)
            .map_err(|_| PqcError::InvalidKeyLength)?;
        hkdf.expand(info, okm)
            .map_err(|_| PqcError::InvalidKeyLength)?;
        Ok(())
...
impl Kdf for HkdfSha3_512 {
    ...
    fn expand(prk: &[u8], info: &[u8], okm: &mut [u8]) -> PqcResult<()> {
        let hkdf = HkdfImpl::<Sha3_512>::from_prk(prk)
            .map_err(|_| PqcError::InvalidKeyLength)?;
        hkdf.expand(info, okm)
            .map_err(|_| PqcError::InvalidKeyLength)?;
        Ok(())
...
mod tests {
    ...
    fn test_password_derivation() {
        ...
        let key3 = helpers::derive_key_from_password(password, b"different salt", iterations).unwrap();
        assert_ne!(&key1[..], &key3[..]);
    }
}