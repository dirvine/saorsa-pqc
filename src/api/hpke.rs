...
use crate::api::kem::{MlKem, MlKemVariant, MlKemPublicKey, MlKemSecretKey, MlKemCiphertext};
...
impl HpkeContext {
    ...
    pub fn seal(&mut self, plaintext: &[u8], aad: &[u8]) -> PqcResult<Vec<u8>> {
        ...
        let ciphertext = self.config.aead.encrypt(
            &self.key[..],
            &nonce,
            plaintext,
            aad,
    ...
    pub fn open(&mut self, ciphertext: &[u8], aad: &[u8]) -> PqcResult<Vec<u8>> {
        let nonce = self.compute_nonce();
        let plaintext = self.config.aead.decrypt(
            &self.key[..],
            &nonce,
            ciphertext,
            aad,
...
impl HpkeSender {
    ...
    pub fn new(config: HpkeConfig) -> Self {
        Self { config }
    }
...
mod tests {
    ...
    fn test_hpke_psk_mode() {
        ...
        // Decrypt with PSK
        let recipient = HpkeRecipient::new(config);
        let mut recipient_ctx = recipient.setup_psk(&enc, &sk.to_bytes(), info, psk, psk_id).unwrap();
        let decrypted = recipient_ctx.open(&ciphertext, aad).unwrap();
