...
use super::{
    params::{
        ...
        SECRET_KEY_SIZE, SIGNATURE_SIZE, TAU,
    },
    polynomial::Polynomial,
    validation::Validator,
    MlDsa65Config, MlDsa65Extended, MlDsa65Operations,
};
use crate::pqc::types::{MlDsaPublicKey, MlDsaSecretKey, MlDsaSignature, PqcError, PqcResult};
...
impl MlDsa65 {
    ...
    fn sample_eta(seed: &[u8]) -> PqcResult<Polynomial> {
        ...
        while coeff_idx < N && seed_idx < seed.len() {
            ...
            if u32::from(val1) < 2 * ETA {
                let coeff = if u32::from(val1) < ETA {
                    u32::from(val1)
                } else {
                    Q - (u32::from(val1) - ETA)
                };
            ...
            if coeff_idx < N && u32::from(val2) < 2 * ETA {
                let coeff = if u32::from(val2) < ETA {
                    u32::from(val2)
                } else {
                    Q - (u32::from(val2) - ETA)
                };
        ...
        if coeff_idx < N {
            return Err(PqcError::CryptoError(
                "Insufficient entropy for eta sampling".to_string(),
            ));
        }
    ...
    fn sample_in_ball(c_tilde: &[u8; 32]) -> PqcResult<Polynomial> {
        ...
        for i in (N - TAU..N).rev() {
            if buffer_idx + 1 >= buffer.len() {
                return Err(PqcError::CryptoError(
                    "Insufficient randomness for challenge sampling".to_string(),
                ));
            }
    ...
    fn sample_mask(k: &[u8; 32], nonce: u16) -> PqcResult<Vec<Polynomial>> {
        ...
        for i in 0..L {
            ...
            for j in 0..N {
                ...
                let val = u32::from_le_bytes([coeff_bytes[0], coeff_bytes[1], coeff_bytes[2], 0])
                    & 0xFFFFF;

                // Map to [-γ1, γ1] range
                if val < 2 * GAMMA1 {
                    poly.coeffs[j] = if val < GAMMA1 {
                        val
                    } else {
                        Q - (val - GAMMA1)
                    };
                    ...
                    poly.coeffs[j] = if retry_val < GAMMA1 {
                        retry_val
                    } else {
                        Q - (retry_val - GAMMA1)
                    };
    ...
    fn make_hint(ct0: &[Polynomial], hint_input: &[Polynomial]) -> PqcResult<Vec<Vec<usize>>> {
    ...
    fn encode_signature(c_tilde: &[u8; 32], z: &Vec<Polynomial>, h: &Vec<Vec<usize>>) -> Vec<u8> {
    ...
    fn use_hint(h: &[Vec<usize>], w_prime: &[Polynomial]) -> PqcResult<Vec<Polynomial>> {
...
impl MlDsa65Operations for MlDsa65 {
    ...
    fn sign(
        ...
        for _attempt in 0..MAX_REJECTION_ATTEMPTS {
            ...
            let w1 = w
                .iter()
                .map(|poly| poly.high_bits(2 * GAMMA2))
                .collect::<Vec<_>>();
        ...
        Err(PqcError::SigningFailed(
            "Maximum rejection attempts exceeded".to_string(),
        ))
    }
...
mod tests {
    ...
    fn test_prehashed_signing() {
        ...
        let signature = ml_dsa
            .sign_prehashed(&sk, &message_hash, message_length, None)
            .unwrap();
        let valid = ml_dsa
            .verify_prehashed(&pk, &message_hash, message_length, &signature, None)
            .unwrap();
    ...
    fn test_configuration() {
        ...
        assert_eq!(
            ml_dsa.config().security.constant_time,
            config.security.constant_time
        );
        assert_eq!(
            ml_dsa.config().performance.enable_simd,
