...
impl Validator {
    ...
    pub fn validate_polynomial_coefficients(coeffs: &[u32], max_val: u32) -> PqcResult<()> {
        ...
        if valid.unwrap_u8() == 0 {
            return Err(PqcError::CryptoError("Invalid polynomial coefficient".to_string()));
        }
    ...
    fn decode_coefficients_3bit(bytes: &[u8]) -> PqcResult<Vec<u32>> {
        ...
        if coeffs.len() != N {
            return Err(PqcError::CryptoError("Incomplete coefficient decoding".to_string()));
        }
    ...
    fn decode_coefficients_variable(bytes: &[u8], bits_per_coeff: usize) -> PqcResult<Vec<u32>> {
        ...
        if coeffs.len() != N {
            return Err(PqcError::CryptoError("Incomplete variable coefficient decoding".to_string()));
        }
