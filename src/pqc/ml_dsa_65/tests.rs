    ...
    fn test_invalid_inputs() {
        ...
        let large_batch = vec![
            (
                pk.clone(),
                b"msg".to_vec(),
                ml_dsa.sign(&sk, b"msg", None).unwrap(),
                None
            );
            MAX_BATCH_SIZE + 1
        ];
...
mod integration_tests {
    ...
    fn test_full_signature_workflow() {
        ...
        for message in messages {
            ...
            // Verify signature
            let is_valid = ml_dsa
                .verify(&public_key, message, &signature, None)
                .unwrap();
    ...
    fn test_cross_context_verification() {
        ...
        let valid_same = ml_dsa
            .verify(&public_key, message, &signature1, Some(context1))
            .unwrap();

        // Verify with different context should fail
        let valid_different = ml_dsa
            .verify(&public_key, message, &signature1, Some(context2))
            .unwrap();
    ...
    fn test_large_scale_operations() {
        ...
        for (message, signature) in &signatures {
            let is_valid = ml_dsa
                .verify(&public_key, message.as_bytes(), signature, None)
                .unwrap();
            assert!(is_valid || !is_valid);
        }

        // Test batch verification
        let batch: Vec<_> = signatures
            .iter()
            .map(|(msg, sig)| {
                (
                    public_key.clone(),
...
mod security_tests {
    ...
    fn test_input_validation() {
        ...
        assert!(ml_dsa
            .sign(&secret_key, b"test", Some(&oversized_context))
            .is_err());
        ...
        let large_batch = vec![
            (
                public_key.clone(),
                b"msg".to_vec(),
                ml_dsa.sign(&secret_key, b"msg", None).unwrap(),
                None
            );
            MAX_BATCH_SIZE + 1
        ];
