...
pub fn sample_uniform(seed: &[u8]) -> PqcResult<Polynomial> {
    ...
    while coeff_count < N {
        ...
        poly.coeffs[coeff_count] =
            u32::conditional_select(&poly.coeffs[coeff_count], &(val1 as u32), accept1);
        coeff_count = ct_increment(accept1, coeff_count as u32) as usize;

        if coeff_count < N {
            poly.coeffs[coeff_count] =
                u32::conditional_select(&poly.coeffs[coeff_count], &(val2 as u32), accept2);
...
pub fn sample_mask(seed: &[u8; 32], nonce: u16) -> PqcResult<Vec<Polynomial>> {
    ...
    for i in 0..L {
        ...
        for j in 0..N {
            ...
            let val =
                u32::from_le_bytes([coeff_bytes[0], coeff_bytes[1], coeff_bytes[2], 0]) & 0xFFFFF;
