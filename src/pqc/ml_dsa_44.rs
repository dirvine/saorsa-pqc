...
use crate::pqc::types::{
    PqcError, PqcResult, ML_DSA_44_PUBLIC_KEY_SIZE, ML_DSA_44_SECRET_KEY_SIZE,
...
impl MlDsa44Operations for MlDsa44 {
    ...
    fn sign(&self, secret_key: &MlDsa44SecretKey, message: &[u8]) -> PqcResult<MlDsa44Signature> {
        let sk_bytes: [u8; ML_DSA_44_SECRET_KEY_SIZE] =
            secret_key
                .as_bytes()
                .try_into()
        ...
        MlDsa44Signature::from_bytes(&sig)
    }
    ...
    fn verify(
        ...
        let pk_bytes: [u8; ML_DSA_44_PUBLIC_KEY_SIZE] =
            public_key
                .as_bytes()
                .try_into()
                .map_err(|_| PqcError::InvalidKeySize {
